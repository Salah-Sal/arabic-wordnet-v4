#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
# setup.sh — إعداد مساحة جمع الشواهد
# Evidence Collection Workspace Setup
# ═══════════════════════════════════════════════════════════════════
#
# Usage:
...
echo ""
echo "═══════════════════════════════════════════════════"
echo "  إعداد مساحة جمع الشواهد — Evidence Collection Setup"
echo "═══════════════════════════════════════════════════"
echo ""
...
if command -v python3 &>/dev/null; then
    ...
    fail "python3 not found. Required for collect_evidence.py."
fi
...
if python3 -c "import yaml" 2>/dev/null; then
    ok "pyyaml package installed"
else
    warn "pyyaml package not found. Installing..."
    pip3 install pyyaml && ok "pyyaml installed" || fail "Failed to install pyyaml"
fi
...
echo ""
echo "  Next steps:"
echo "  1. Collect evidence for a synset:"
echo "     python3 tools/collect_evidence.py awn4-05162506-n"
echo ""
echo "  2. Browse synsets:"
echo "     python3 tools/extract_synset_wn.py --random 5"
echo ""
echo "  3. Run a batch:"
echo "     python3 tools/collect_evidence.py --batch batches/my_batch.txt"
echo ""
echo "  4. Interactive SQL:"
