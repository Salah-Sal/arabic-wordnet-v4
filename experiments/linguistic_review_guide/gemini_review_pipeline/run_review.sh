#!/bin/bash
# run_review.sh — Batch driver for gemini_review_pipeline v2 (5-stage, DB-direct).
#
# Usage:
...
#      (or use the symlinked prepared/ from claude_code_db/)
#   2. Gemini CLI must be installed (npm i -g @google/gemini-cli)
#   3. GEMINI_API_KEY must be set (or OAuth credentials in ~/.gemini/)
...
OUTPUT_DIR="${OUTPUT_DIR:-$GUIDE_DIR/output/reviews_gemini_pipeline_v2}"
...
if [ "${1:-}" = "--all" ]; then
    ...
    echo "  OUTPUT_DIR       Output directory (default: ../output/reviews_gemini_pipeline_v2)"
    exit 1
fi
...
echo "=== AWN4 Gemini Review Pipeline v2 (5-Stage, DB-Direct) ==="
echo "Model:     $MODEL"
...
for SYNSET_ID in $SYNSETS; do
    ...
    echo "[$TOTAL] Reviewing $SYNSET_ID (pipeline v2)..."
...
- Masked synset info (for Stage 1 blind generation ONLY): ${MASKED_PATH}
- Output template: ${SPEC_DIR}/output_template.yaml
...
Write the complete review YAML to: ${REVIEW_PATH}"
    ...
    # --yolo is the CLI shorthand for --approval-mode yolo (skip all permission prompts).
    # --include-directories adds directories to Gemini's workspace sandbox so that
    # read_file/write_file can access them. We need both:
    #   - prepared/ (for reading synset_info)
    #   - OUTPUT_DIR (for writing the review YAML)
    RESOLVED_PREPARED="$(cd "$PREPARED_DIR" 2>/dev/null && pwd)"
    RESOLVED_OUTPUT="$(cd "$OUTPUT_DIR" 2>/dev/null && pwd)"
    GEMINI_ARGS=(
        gemini
        ...
        --include-directories "$RESOLVED_OUTPUT"
    )
    ...
    if "${GEMINI_ARGS[@]}" -p "$USER_PROMPT" > "$TRAJECTORY_PATH" 2>"$OUTPUT_DIR/${SYNSET_ID}.stderr.log"; then
        ...
        if [ -f "$REVIEW_PATH" ]; then
            ...
            if [ -n "$RESULT_LINE" ]; then
                ...
                # Gemini pricing: $1.25/M input, $10.00/M output
