#!/usr/bin/env python3
"""Extract synset_info + masked variant for review_pipeline v2.

Simplified fork of claude_code_db/extract_synset_info.py:
- Keeps: WNBridge, ArabicDumper, normalize_arabic, synset YAML generation
- Removes: EvidencePrefetcher (evidence is now gathered in-session at Stage 2)

Produces lightweight prepared/{synset_id}/ directories containing:
  - synset_info.yaml       (full synset metadata with lemmas)
  - synset_info_masked.yaml (lemmas removed — for Stage 1 blind generation)
...
def mask_synset_info(synset_info_yaml: str) -> str:
    """Remove lemma lists for unbiased lemma generation (Stage 1)."""
