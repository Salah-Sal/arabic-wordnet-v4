#!/bin/bash
# run_review.sh — Batch driver for review_pipeline v2 (5-stage, DB-direct).
#
# Usage:
#   ./run_review.sh awn4-02592253-n                # single synset
#   ./run_review.sh --all                           # all prepared synsets
#   MODEL=haiku MAX_TURNS=80 ./run_review.sh --all  # custom settings
#
# Prerequisites:
...
if [ "${1:-}" = "--all" ]; then
    ...
    echo "  OUTPUT_DIR       Output directory (default: ../output/reviews_pipeline_v2)"
...
echo "=== AWN4 Review Pipeline v2 (5-Stage, DB-Direct) ==="
echo "Model:     $MODEL"
