...
# batch_status.py lives alongside this file (symlinked from claude_code_db)
    ...
    def __init__(
        ...
        inside Docker, ignored in standalone mode):
            OUTPUT_DIR: Review output directory. Default for standalone use
                is ``output/reviews_gemini_db``; the orchestrator overrides
                this per-wave (e.g., ``output/reviews/W2``).
        ...
        self.output_dir = Path(
            os.environ.get("OUTPUT_DIR", str(guide_dir / "output" / "reviews_gemini_db"))
        )
...
""",
    ...
    output_dir = Path(os.environ.get("OUTPUT_DIR", str(guide_dir / "output" / "reviews_gemini_db")))
