#!/usr/bin/env python3
"""Finetuned Jina v5-text-nano v2 + FAISS backend.

Loads finetuned weights from HF Hub (SalahAbdoNLP/jina-v5-nano-arabic-dict-v2)
...
v2 changes from v1:
  - CachedMultipleNegativesRankingLoss (GradCache) instead of plain MNRL
  - Anchor decomposition: each anchor → 3 variants (combined, lemma-only, def-only)
  - 13,980 training triplets (3× v1's 4,660)
  - fp16 instead of bf16, trained on Kaggle 2×T4
...
_FT_LOCAL = (Path(__file__).resolve().parent.parent.parent
             / "trajectory_dataset" / "output" / "jina_v5_nano_finetuned_v2")
_FT_REPO = "SalahAbdoNLP/jina-v5-nano-arabic-dict-v2"
_BASE_REPO = "jinaai/jina-embeddings-v5-text-nano-retrieval"
_DIMENSIONS = 768
_RUN_NAME = "jina_v5_nano_finetuned_v2"
...
def _prepare_model_dir() -> Path:
    ...
    merged_dir = Path.home() / ".cache" / "jina_v5_nano_finetuned_v2_merged"
...
def _load_model():
    ...
    print(f"Loading finetuned v2 model from {model_dir}...", file=sys.stderr)
...
def setup(export_dir, **kwargs) -> dict:
    ...
    print(f"Embedding {len(files)} files with finetuned v5-nano v2 "
